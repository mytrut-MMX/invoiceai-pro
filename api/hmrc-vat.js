/**
 * HMRC MTD VAT endpoint — obligations, submission, retrieval.
 *
 * Endpoints:
 *   GET  ?action=obligations&vrn={vrn}                → list VAT periods
 *   POST  action=submit, body: { vrn, periodKey, box1-box9, finalised }  → submit return
...
export default async function handler(req, res) {
  ...
  const rl = checkRateLimit(`hmrc-vat:${user.id}`, USER_RATE_LIMIT);
  ...
  if (req.method === 'POST' && action === 'submit') {
    ...
    // Parse and validate all 9 boxes
    const boxes = {};
    ...
    // Server-side validation: box3 = box1 + box2
    const expectedBox3 = Math.round((boxes.box1 + boxes.box2) * 100) / 100;
    if (boxes.box3 !== expectedBox3) {
      return res.status(400).json({ error: `Box 3 must equal Box 1 + Box 2. Expected ${expectedBox3}, got ${boxes.box3}.` });
    }

    // Server-side validation: box5 = box3 - box4
    const expectedBox5 = Math.round((boxes.box3 - boxes.box4) * 100) / 100;
    if (boxes.box5 !== expectedBox5) {
      return res.status(400).json({ error: `Box 5 must equal Box 3 - Box 4. Expected ${expectedBox5}, got ${boxes.box5}.` });
    }

    // Build HMRC payload per MTD spec
    const hmrcPayload = {
      periodKey,
    ...
    if (periodStart && periodEnd) {
      // Upsert vat_periods row → status=submitted, locked=true
      ...
      // Record full submission in vat_return_submissions
      if (periodRow?.id) {
