...
export default function BackendIntegrationsLeadPanel({ token, prefillTask, setPrefillTask, onTaskCompleted }) {
  ...
  const copyAsCCPrompt = async () => {
    try { await navigator.clipboard.writeText(specToPrompt('Backend & Integrations Lead', spec)); } catch {}
    setCopied(true);
      ...
      {spec && (
        <div style={{ marginTop:16 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12, marginBottom:12, flexWrap:'wrap' }}>
            <div style={{ fontSize:13, fontWeight:700, color:'#0F172A' }}>Backend & Integrations Lead Spec</div>
