...
const TABS = ["Basic Details", "Address & Payment", "Tax Details", "Contact Persons", "Custom Fields", "Remarks"];
...
export default function CustomerModal({ existing, onClose, onSave }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:20 }}>
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:860, maxHeight:"92vh", display:"flex", flexDirection:"column", boxShadow:"0 20px 60px rgba(0,0,0,0.18)", fontFamily:ff }}>
        ...
        <div style={{ padding:"10px 24px", borderBottom:"1px solid #F0F0F0", display:"flex", gap:8, overflowX:"auto" }}>
          {TABS.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{ border:"1px solid #E5E7EB", background:activeTab===tab?"#1A1A1A":"#fff", color:activeTab===tab?"#fff":"#555", padding:"7px 12px", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>
              {tab}
            </button>
          ))}
        </div>
                ...
                <>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 12px", background:"#F9F9F9", borderRadius:8, border:"1px solid #EBEBEB", marginBottom:12 }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:"#AAA", marginTop:1 }}>Set whether this customer has VAT registration.</div>
                    </div>
                    <Switch checked={customerVatRegistered} onChange={(v) => { setCustomerVatRegistered(v); if (!v) setVatNumber(""); }} />
                  </div>

                  {customerVatRegistered && (
                    <Field label="VAT Number" hint="Use format like GB123456789">
            ...
            <div style={{ paddingTop:8 }}>
              {customFields.map(f=>(
                <div key={f.id} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 28px", gap:8, marginBottom:8, alignItems:"flex-end" }}>
                  ...
                  <button onClick={()=>delCF(f.id)} style={{ height:38, border:"none", background:"none", cursor:"pointer", color:"#CCC", marginBottom:14 }} onMouseEnter={e=>e.currentTarget.style.color="#DC2626"} onMouseLeave={e=>e.currentTarget.style.color="#CCC"}><Icons.Trash /></button>
                </div>
              ))}
