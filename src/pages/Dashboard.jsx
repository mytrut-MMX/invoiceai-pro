...
import { InvoicePreview, TemplateSelector, exportToPDF } from '../components/InvoicePreview.jsx'
...
export default function Dashboard({ store }) {
  const { company, invoices, setInvoices, clients, setClients, products, setProducts, settings, setSettings, templates, setTemplates, selectedTemplateId, setSelectedTemplateId, selectedTemplate, genId, today, due, nextInvoiceNum } = store
  ...
  // Stats
  const allInvoices = invoices.filter(i => i.doc_type !== 'quote')
  const allQuotes   = invoices.filter(i => i.doc_type === 'quote')
  const totalPaid   = allInvoices.filter(i => i.status === 'paid').reduce((s, i) => s + (i.total || 0), 0)
  const totalDue    = allInvoices.filter(i => ['sent','partial'].includes(i.status)).reduce((s, i) => s + (i.total || 0), 0)
  const totalOverdue= allInvoices.filter(i => i.status === 'overdue').reduce((s, i) => s + (i.total || 0), 0)
  const thisMonth   = allInvoices.filter(i => i.date?.startsWith(new Date().toISOString().slice(0,7)))
  ...
  const calcTotals = (data) => {
    ...
    const vatTotal = items.reduce((s, i) => {
      ...
      const vr = Number(i.vat_rate ?? data.tax_rate ?? (company.vat_registered === 'yes' ? (company.tax_rate || 20) : 0))
  ...
  const saveInvoice = (data) => {
    ...
    if (editingInvoice) {
      ...
      const newInv = { id: genId(), number: nextInvoiceNum(data.doc_type), date: today(), due_date: due(company.payment_terms), ...data, ...totals }
  ...
  const openNew = (docType = 'invoice') => {
    setDefaultDocType(docType); setEditingInvoice(null); setAiDraft(null); setShowEditor(true)
  }
  ...
  const addTemplate = async (e) => {
    ...
    for (const file of files) {
      if (!file.type.startsWith('image/')) { alert(`${file.name}: only image files supported (PNG, JPG). Convert PDF to image first.`); continue }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", background: '#f7f4ef' }}>
        ...
        <nav style={{ flex: 1, padding: '0 10px' }}>
          {NAV.map(n => (
            <button key={n.id} onClick={() => setPage(n.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', marginBottom: 2, textAlign: 'left', background: page === n.id ? '#e2b96a15' : 'transparent', color: page === n.id ? '#e2b96a' : '#94a3b8', fontWeight: page === n.id ? 700 : 400, fontSize: 14, borderLeft: page === n.id ? '3px solid #e2b96a' : '3px solid transparent' }}>
              ...
              {n.id === 'invoices' && allInvoices.filter(i => i.status === 'overdue').length > 0 && <span style={{ marginLeft: 'auto', background: '#dc2626', color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: 10, fontWeight: 700 }}>{allInvoices.filter(i => i.status === 'overdue').length}</span>}
            </button>
          ))}
        </nav>
        <div style={{ padding: 12 }}>
          <button onClick={() => setShowAI(!showAI)} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid #e2b96a44', cursor: 'pointer', background: showAI ? '#e2b96a22' : 'transparent', color: '#e2b96a', fontWeight: 700, fontSize: 13 }}>✨ AI Assistant</button>
        </div>
      </div>
      ...
      <div style={{ marginLeft: 220, flex: 1, padding: 28, minHeight: '100vh', paddingRight: showAI ? 360 : 28 }}>
        ...
        {page === 'dashboard' && (
          <div>
            ...
            <div style={{ background: '#fff', borderRadius: 14, padding: 20, border: '1px solid #f1f5f9' }}>
              ...
              <DocTable docs={allInvoices.slice(0, 6)} company={company} onEdit={openEdit} onPreview={setPreviewInvoice} onDelete={deleteDoc} onEmail={sendEmail} onStatusChange={(id, st) => setInvoices(p => p.map(i => i.id === id ? { ...i, status: st } : i))} />
            </div>
          </div>
        )}
        ...
        {/* ── QUOTES ── */}
        {page === 'quotes' && (
          <PageWithDocs title="Quotes" docs={filteredDocs('quote')} docType="quote" company={company} search={search} setSearch={setSearch} filterStatus={filterStatus} setFilterStatus={setFilterStatus} onNew={() => openNew('quote')} onEdit={openEdit} onPreview={setPreviewInvoice} onDelete={deleteDoc} onEmail={sendEmail} setInvoices={setInvoices} />
        )}
      ...
      {showEditor && (
        <InvoiceEditor
          invoice={editingInvoice
            ? { ...editingInvoice }
            : aiDraft
              ? { number: nextInvoiceNum(aiDraft.doc_type || defaultDocType), date: today(), due_date: due(company.payment_terms), status: 'draft', doc_type: defaultDocType, currency: company.currency, tax_rate: company.vat_registered === 'yes' ? (Number(company.tax_rate) || 20) : 0, ...aiDraft }
              : null}
      ...
      {previewInvoice && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,15,30,0.92)', zIndex: 1100, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <InvoicePreview invoice={previewInvoice} company={company} template={selectedTemplate} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
        ...
        <button onClick={onNew} style={{ padding: '9px 18px', borderRadius: 10, border: 'none', background: '#1a1a2e', color: '#e2b96a', fontWeight: 700, cursor: 'pointer' }}>+ New {docType === 'quote' ? 'Quote' : 'Invoice'}</button>
      </div>
...
function ClientsPage({ clients, setClients, genId, invoices, company }) {
  ...
  const editC = c => { setForm(c); setEditing(c.id); setShowForm(true) }
            ...
            <div key={c.id} style={{ background: '#fff', borderRadius: 14, padding: 18, border: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => editC(c)} style={{ width: 27, height: 27, borderRadius: 6, border: 'none', background: '#f1f5f9', cursor: 'pointer', fontSize: 12 }}>✏️</button>
...
// ── Settings Page ─────────────────────────────────────────────────────────────
function SettingsPage({ company, settings, setSettings, templates, selectedTemplateId, onSelectTemplate, onAddTemplate, onRemoveTemplate }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: 22, border: '1px solid #f1f5f9' }}>
          ...
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: 10, fontSize: 12, color: '#92400e' }}>
            Template variables: <code>to_email, to_name, invoice_number, amount, due_date, from_name</code>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: 22, border: '1px solid #f1f5f9' }}>
          ...
          <F label="Anthropic API Key" k="anthropic_key" placeholder="sk-ant-..." type="password" hint="console.anthropic.com — $5 free credit on signup" />
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: 22, border: '1px solid #f1f5f9', gridColumn: '1/-1' }}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8, fontFamily: "'Playfair Display',serif", color: '#1a1a2e' }}>🎨 Invoice Templates</div>
          <p style={{ color: '#64748b', fontSize: 13, marginBottom: 16 }}>
            Upload your own branded templates as PNG/JPG images. They will be used as the full background of your invoices. Select the active template by clicking on it.
            <br /><span style={{ color: '#e2b96a', fontWeight: 600 }}>💡 To use a PDF template: open in browser, screenshot it, save as PNG.</span>
          </p>
          <TemplateSelector
            templates={templates}
          ...
          <div style={{ marginTop: 10, fontSize: 12, color: '#94a3b8' }}>
            Active template: <strong style={{ color: '#334155' }}>{selectedTemplateId === 'default' ? 'Default (built-in)' : templates.find(t => t.id === selectedTemplateId)?.name || 'Unknown'}</strong>
          </div>
        </div>
      </div>
