...
function drawInvoicesTable(doc, rows, y) {
  const widthW = CR - ML;
  const cols = [
    { label: "Reference",    w: 0.22, align: "left"  },
    { label: "Invoice date", w: 0.13, align: "left"  },
    { label: "Payment date", w: 0.13, align: "left"  },
    { label: "Gross (A)",    w: 0.09, align: "right" },
    { label: "Materials",    w: 0.09, align: "right" },
    { label: "Non-CIS",      w: 0.08, align: "right" },
    { label: "Labour",       w: 0.09, align: "right" },
    { label: "CIS (B)",      w: 0.08, align: "right" },
    { label: "Paid (A - B)", w: 0.09, align: "right" },
  ];
  ...
  const colWidths = cols.map((c) => c.w * widthW);
  const rightEdges = colX.map((x, i) => x + colWidths[i]);
  ...
  for (const r of rows) {
    ...
    cols.forEach((c, i) => {
      const cellW = colWidths[i] - hPad * 2;
      const tx = c.align === "right" ? rightEdges[i] - hPad : colX[i] + hPad;
      const opts = c.align === "right"
        ? { align: "right", maxWidth: cellW }
        : { maxWidth: cellW };
      doc.text(String(values[i]), tx, rowBase, opts);
    });
    y += rowH;
    doc.line(ML, y, CR, y);
  }
 ...
 *   - disclaimer rule at pageH - 30     (y = 267)
 *   - disclaimer baseline at pageH - 27 (y = 270), 3.5mm line spacing
 *   - page strip baseline at pageH - 12 (y = 285)
 */
function drawPinnedFooter(doc, contractorName, { skipDisclaimer = false } = {}) {
  if (!skipDisclaimer) {
    setLinePen(doc);
    doc.line(ML, PAGE_H - 30, CR, PAGE_H - 30);
    ...
    const discBase = PAGE_H - 27;
    wrapped.forEach((line, i) => {
      doc.text(line, ML, discBase + i * 3.5);
    });
  }
...
function buildDoc({ contractor = {}, subcontractor = {}, period = {}, amounts = {}, invoices }, logoDataUrl = "") {
  ...
  y = drawSectionHeading(doc, "Subcontractor details", y) + 5; // heading → name: 5mm

  const nameTopY = y;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(F_BODY);
  doc.setTextColor(17, 17, 17);
  doc.text(subcontractor.name || "\u2014", ML, nameTopY + ascent(F_BODY));

  // Underline under subcontractor name, spanning left column width
  setLinePen(doc);
  doc.line(ML, nameTopY + 3, leftXR, nameTopY + 3);
